...
func RegisterUserLoginHandler(mux *http.ServeMux, cfg *config.ApiConfig) {
	mux.HandleFunc("POST /api/login", userLoginHandler(cfg))
}
...
type userAccountResponseData struct {
	ID        string    `json:"id"`
...
func createUserHandler(cfg *config.ApiConfig) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		newUser := userAccountResponseData{
			ID:        data.ID.String(),
...
func userLoginHandler(cfg *config.ApiConfig) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		params := userRegisterLogin{}
		decoder := json.NewDecoder(r.Body)
		if err := decoder.Decode(&params); err != nil {
		...
		successData := userAccountResponseData{
		...
		util.RespondWithJSON(w, http.StatusOK, successData)
